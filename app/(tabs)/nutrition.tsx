...
export default function NutritionScreen() {
  ...
  const calculateHealthScore = (entry: any): { score: number; color: string; label: string } => {
    ...
    if (totalMacroCals === 0 || calories === 0) {
    ...
    const isLikelyWholeFruit = carbsPercent > 80 && fatPercent < 5 && proteinPercent < 10 && calories < 150;
    const isLikelyVegetable = carbsPercent > 50 && fatPercent < 10 && calories < 100;
    const isLikelyNuts = fatPercent > 60 && proteinPercent > 10;
    const isLikelyLeanProtein = proteinPercent > 60 && fatPercent < 30;
    
    if (isLikelyWholeFruit) {
      score = 8;
    } else if (isLikelyVegetable) {
      score = 9;
    } else if (isLikelyNuts) {
      score = 7;
    } else if (isLikelyLeanProtein) {
      score = 8;
    } else {
      ...
      if (fatPercent >= 20 && fatPercent <= 40) {
        ...
        score -= 1;
      } else if (fatPercent < 5 && !isLikelyWholeFruit) {
        score -= 0.5;
      }
      
      if (carbsPercent > 80 && !isLikelyWholeFruit) {
        score -= 1;
      } else if (carbsPercent >= 40 && carbsPercent <= 60) {
        score += 0.5;
      }
    }
    ...
    if (caloriesPerGram < 1) {
      score += 1;
    } else if (caloriesPerGram < 2) {
      score += 0.5;
    } else if (caloriesPerGram > 5) {
      score -= 0.5;
    }
    
    const foodName = entry.name?.toLowerCase() || '';
    if (foodName.includes('honey') || foodName.includes('apple')) {
      score += 1;
    }
    
    if (foodName.includes('fried') || foodName.includes('deep') || foodName.includes('candy')) {
      score -= 1.5;
    } else if (foodName.includes('cake') || foodName.includes('cookie') || foodName.includes('donut')) {
      score -= 1;
    }
